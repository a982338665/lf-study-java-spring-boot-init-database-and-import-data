...
#2.参数值由pom文件传递======================================================
#压缩包名称 publish-test.zip的publish
baseZipName="${package-name}-${activeProfile}"
#命令启动包名 xx.jar的xx
packageName="${package-name}"
#java -cp启动时，指定main入口类;命令：java -cp conf;lib\*.jar;${packageName}.jar ${mainclass}
mainclass="${boot-main}"
application_options="-DXms=100M"
...
function start()
{
    ...
    then
        ...
        then
            if [ ${mainclass} ]
            then
              # -cp conf:lib\*  用于linux ，用于windows时换成 ;
                echo "java $application_options -cp conf:lib\*.jar:${packageName}.jar ${mainclass} >${baseDirPath}/${packageName}.out 2>&1 &"
                nohup java $application_options -cp conf:lib\*.jar:${packageName}.jar ${mainclass} >${baseDirPath}/${packageName}.out 2>&1 &
               #nohup java -cp conf:lib\*.jar:${packageName}.jar ${mainclass} >/dev/null 2>&1 &
            fi
        elif [ "${languageType}" == "java" ]
        then
             echo "java -jar $application_options ${baseDirPath}/${baseZipName}/${packageName}.jar >/dev/null 2>&1 &"
            nohup java -jar $application_options ${baseDirPath}/${baseZipName}/${packageName}.jar >/dev/null 2>&1 &
        ...
        then
            ...
            echo ${baseDirPath}/${baseZipName}/${packageName} >/dev/null 2>&1 &
