package pers.li.magictest.config;
 ...
 */
@Configuration
//当存在类 RequestMappingHandlerMapping 时，此类才生效，所属jar为 spring-webmvc-5.3.6.jar
@ConditionalOnClass({RequestMappingHandlerMapping.class})
//通过此注解 将 MagicAPIProperties 类注入ioc容器
@EnableConfigurationProperties(MagicAPIProperties.class)
//将数组中的类全部导入交给spring管理
...
//开启websocket
@EnableWebSocket
//表示在 MagicPluginConfiguration 加载之后，再加载当前类
...
public class MagicAPIAutoConfiguration implements WebMvcConfigurer, WebSocketConfigurer {
     ...
     * 注册模块、类型扩展
     *
     * @param extensionMethods  拓展方法
     * @param languageProviders 自定义脚本语言支持
     */
...
//        Function<String, BiFunction<Map<String, Object>, String, Object>> loader
        MagicResourceLoader.addScriptLanguageLoader(language -> languageProviders.stream()
                        ...
                        .findFirst().<BiFunction<Map<String, Object>, String, Object>>map(languageProvider ->
//                        传入map 和 String 返回 Object
                                (context, script) -> {
                                    try {
